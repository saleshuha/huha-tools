-- Step 1: Clean up existing PO data
DELETE FROM public.po_upload_job_errors WHERE job_id IN (
  SELECT id FROM public.po_upload_jobs WHERE user_id = auth.uid()
);
DELETE FROM public.po_upload_jobs WHERE user_id = auth.uid();
DELETE FROM public.po_orders WHERE user_id = auth.uid();

-- Step 2: Add computed columns for normalized identity
ALTER TABLE public.po_orders 
ADD COLUMN po_key text GENERATED ALWAYS AS (lower(trim(po_number))) STORED,
ADD COLUMN item_key text GENERATED ALWAYS AS (lower(trim(coalesce(nullif(model_number, ''), nullif(asin, ''), sku_code)))) STORED;
...
BEGIN
  ...
  ORDER BY po.po_key, po.item_key, 
    CASE po.status
      WHEN 'closed' THEN 1
...
-- Step 5: Update dashboard metrics to use identity-based deduplication
