...
BEGIN
  ...
  IF p_item_title IS NOT NULL AND p_item_title != '' THEN
    ...
    IF v_category IS NOT NULL THEN
      ...
      -- STEP 2: Brand range full (or not found) — scan ALL brand ranges in this category for gaps
      FOR v_cat_range IN
      ...
      LOOP
        FOR v_next_serial IN v_cat_range.range_start..v_cat_range.range_end LOOP
          ...
          IF NOT v_is_duplicate AND NOT v_is_in_additional THEN
            -- Found a gap in another brand's range within same category
            UPDATE serial_range_directory
            SET items_used = items_used + 1, updated_at = now()
      ...
      -- STEP 3: Entire category is full — extend with new block
      ...
      -- Use the original brand range_id if available, else create extension
      IF v_range_id IS NOT NULL THEN
        UPDATE serial_range_directory
