-- Fix case-sensitive type matching in get_printable_orders function
CREATE OR REPLACE FUNCTION public.get_printable_orders(start_date timestamp with time zone, end_date timestamp with time zone, date_filter_type text DEFAULT 'created_at'::text, status_filter text DEFAULT 'all'::text)
 RETURNS TABLE(id uuid, order_id text, asin text, sku text, item_title text, item_quantity integer, order_place_date text, order_status text, created_at timestamp with time zone, source_file text, printable boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
