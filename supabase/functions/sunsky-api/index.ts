...
// Background job processor for itemNos
async function processImportJob(job: any, userId: string, userCountry: string) {
  try {
    ...
    if (type === 'itemNos') {
      await processItemNosJob(job, userId, userCountry, criteria.itemNos, credentials);
    } else {
      await processLegacyImportJob(job, userId, userCountry, credentials);
    }
...
// Process itemNos job (for selected SKUs import)
async function processItemNosJob(job: any, userId: string, userCountry: string, itemNos: string[], credentials: any) {
  let successCount = 0;
  let errorCount = 0;
  ...
  for (let i = 0; i < itemNos.length; i++) {
    ...
    try {
      ...
      console.error(`Error processing item ${itemNo}:`, error);
      errorCount++;
    }
...
// Process PO search job
async function processPoSearchJob(job: any, userId: string, userCountry: string, modelNumbers: string[]) {
  try {
    const credentials = await getApiCredentials(userId);
      ...
      try {
        ...
        if (modelNumber.match(/^[A-Z0-9]{6,}$/i)) {
          try {
            ...
            if (detailResult.result === 'success' && detailResult.data) {
              productToImport = detailResult.data;
              matchFound = true;
            }
          } catch (error) {
            ...
            if (searchResult.result === 'success' && searchResult.data?.result?.length > 0) {
              ...
              if (bestMatch) {
                ...
                try {
                  const detailResult = await makeSunskyRequest(
                    '/openapi/product!detail.do',
                    { lang: 'en', itemNo: bestMatch.itemNo },
                    credentials.key,
                    credentials.secret,
                    userId
                  );
        ...
        if (matchFound && productToImport) {
          // Convert price to user's currency
          const convertedCost = await convertCurrency(
            parseFloat(productToImport.price || 0),
            userCountry
          );
          ...
          const { error: skuError } = await supabase
            ...
            .upsert({
              ...
              cost: convertedCost,
              weight: productToImport.unitWeight ? parseFloat(productToImport.unitWeight) : null,
              description: `Imported from Sunsky - Lead Time: ${productToImport.leadTime || 'N/A'}`,
              currency: userCountry === 'KSA' ? 'SAR' : 'AED',
              country: userCountry,
          ...
          if (!skuError) {
            // Update PO orders with matched SKU
            await supabase
              .from('po_orders')
              .update({
                sku_code: productToImport.itemNo,
        ...
        console.error(`Error processing model ${modelNumber}:`, error);
        errorCount++;
      }
    ...
    console.error(`Error processing PO search job ${job.id}:`, error);
...
// Legacy import job processor (for existing category/brand jobs)
async function processLegacyImportJob(job: any, userId: string, userCountry: string, credentials: any) {
  let products: any[] = [];
  const { type, criteria } = job;
...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'createItemNosJob': {
        const { itemNos } = data;
        
        if (!itemNos || !Array.isArray(itemNos) || itemNos.length === 0) {
          return new Response(JSON.stringify({
            result: 'error',
            message: 'Missing required field: itemNos array'
          }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
        ...
        try {
          ...
          // Start background processing
          EdgeRuntime.waitUntil(processImportJob(job, user.id, user.country));
      ...
      case 'createPoSearchJob': {
        try {
          // Get PO model numbers from database
          ...
          if (!poOrders || poOrders.length === 0) {
            return new Response(JSON.stringify({
              result: 'error',
              message: 'No model numbers found in PO orders'
            }), {
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            });
          }
          ...
          const modelNumbers = [...new Set(poOrders
            .map(po => po.model_number?.trim())
          ...
          if (modelNumbers.length === 0) {
            return new Response(JSON.stringify({
              result: 'error',
              message: 'No valid model numbers found'
            }), {
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            });
          }
          ...
          const { data: job, error: jobError } = await supabase
            ...
            .insert({
              ...
              criteria: { 
                ...
                total_models: poOrders.length,
                unique_models: modelNumbers.length
              },
              status: 'queued',
          ...
          // Start background processing
          EdgeRuntime.waitUntil(processPoSearchJob(job, user.id, user.country, modelNumbers));

          return new Response(JSON.stringify({
            result: 'success',
            data: job,
            message: 'Background PO search job started'
          }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        } catch (error) {
          ...
          return new Response(JSON.stringify({
            result: 'error',
            message: error.message || 'Failed to create PO search job'
          }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
