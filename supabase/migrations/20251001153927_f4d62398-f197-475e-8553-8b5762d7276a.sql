...
BEGIN
  ...
  SELECT 
    ...
    CASE
      WHEN t.total_sold > 0 AND EXTRACT(days FROM (now() - ib.first_added_date)) > 0 THEN 
        GREATEST(1, CEIL((t.total_sold / EXTRACT(days FROM (now() - ib.first_added_date))::numeric) * 30))::integer
      ELSE 0
    END as recommended_quantity,
    CASE 
      WHEN EXTRACT(days FROM (now() - ib.first_added_date)) > 0
      THEN t.total_sold / EXTRACT(days FROM (now() - ib.first_added_date))::numeric
      ELSE 0
