-- Fix any remaining SECURITY DEFINER functions without search_path
-- Update existing functions that may be missing search_path
CREATE OR REPLACE FUNCTION public.get_noon_sales_upload_summary(country_filter text DEFAULT NULL::text)
 RETURNS TABLE(id uuid, store_name text, report_month text, upload_date timestamp with time zone, record_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.get_order_fees_analysis(country_filter text DEFAULT 'UAE'::text, store_filter uuid DEFAULT NULL::uuid, start_date timestamp with time zone DEFAULT (now() - '3 mons'::interval), end_date timestamp with time zone DEFAULT now())
 RETURNS TABLE(order_number text, order_type text, item_nr text, sku text, description text, document_date timestamp with time zone, invoice_price numeric, total_fees numeric, net_amount numeric, fee_breakdown jsonb, fee_coverage_status text, order_status text, store_name text, profit_margin numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
