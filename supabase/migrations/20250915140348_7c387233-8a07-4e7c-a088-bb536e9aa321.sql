-- Update the velocity analysis function to provide better recommendations for items without sales history
...
BEGIN
  ...
  SELECT 
    ...
    -- Improved recommended reorder quantity logic
    CASE 
      -- For items with good sales velocity
      ...
      -- For items with no sales but based on inventory patterns
      WHEN vd.current_quantity = 0 THEN 5  -- Immediate restock needed
      WHEN vd.current_quantity <= 2 THEN 3  -- Low stock items
      WHEN vd.total_sales = 0 AND EXTRACT(days FROM now() - vd.date_added) > 30 THEN 2  -- Old items with no sales
      ELSE 1  -- Default minimum
    END::integer as recommended_reorder_quantity,
    
    -- Reorder point (when to reorder)
    CASE 
      WHEN vd.sales_velocity > 0 THEN GREATEST(1, CEIL(vd.sales_velocity * 7))
      ELSE GREATEST(1, vd.current_quantity / 2)  -- Reorder at half current stock if no velocity data
    ...
    CASE 
      ...
      WHEN vd.current_quantity <= 1 THEN 80  -- High urgency for low stock even without sales data
      WHEN vd.current_quantity <= 2 THEN 60  -- Medium urgency for low stock
      ELSE 30
