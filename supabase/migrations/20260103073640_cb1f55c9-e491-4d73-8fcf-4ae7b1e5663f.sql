...
-- Create comprehensive performance analysis function
-- Calculates lifetime performance metrics for ALL inventory items
...
BEGIN
  ...
  WITH stock_changes_agg AS (
    -- Aggregate stock changes per item
    SELECT 
      sc.inventory_id,
      -- Total units added (positive changes)
      COALESCE(SUM(CASE WHEN sc.change_amount > 0 THEN sc.change_amount ELSE 0 END), 0) as total_added,
      -- Total units removed (negative changes = sales)
      ...
      -- Last sale date (most recent negative change)
      ...
      -- Count restock events for calculating average sellout time
  ...
  sellout_times AS (
    -- Calculate average time between restock and sellout
    SELECT 
      sc.inventory_id,
      AVG(
        CASE 
  ...
  SELECT 
    ...
    CASE 
      ...
      ELSE LEAST(100, GREATEST(0, (
        -- 40% weight: Recent velocity (7d compared to 30d trend)
        (CASE 
