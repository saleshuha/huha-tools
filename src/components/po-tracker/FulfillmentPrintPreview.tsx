  ...
  const handlePrint = () => {
    const rows = sortedOrders.map((order, idx) => {
      ...
      return `<tr style="background:${bgColor}">
        ...
        <td><span>${order.asin || '—'}</span>${order.sku_code || order.model_number ? ` <span style="color:#888">${order.sku_code || order.model_number}</span>` : ''}</td>
        <td style="white-space:normal;word-break:break-word;max-width:220px">${order.title || '—'}</td>
        <td>${poNumbers}</td>
        ...
        <td style="text-align:center">${order._fulfillmentStatus}</td>
      </tr>`;
    }).join('');

    const html = `<!DOCTYPE html><html><head><title>Fulfillment Report</title><style>
      @page { size: landscape; margin: 6mm; }
      body { font-family: Arial, Helvetica, sans-serif; font-size: 7pt; margin: 0; padding: 0; color: #000; }
      .report-header { margin-bottom: 4px; padding-bottom: 3px; border-bottom: 2px solid #000; }
      .report-header h1 { font-size: 10pt; margin: 0 0 1px 0; font-weight: 700; }
      .report-header p { font-size: 7pt; margin: 0; color: #555; }
      table { width: 100%; border-collapse: collapse; table-layout: auto; }
      th, td { padding: 1px 4px; border: 1px solid #444; font-size: 7pt; white-space: nowrap; line-height: 1.2; }
      th { background: #ddd; font-weight: bold; text-align: center; }
      tr { page-break-inside: avoid; }
      .footer-note { margin-top: 4px; font-size: 6pt; color: #888; text-align: right; }
    </style></head><body>
      <div class="report-header">
        <h1>Fulfilled Items Preview — In-Stock Inventory Process</h1>
        <p>${totals.items} items · ${new Date().toLocaleDateString()}</p>
      </div>
      <table>
        <thead><tr>
