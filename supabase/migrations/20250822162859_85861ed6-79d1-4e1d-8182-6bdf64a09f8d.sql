-- Create a new unlimited version of the PO orders function
CREATE OR REPLACE FUNCTION public.get_all_po_orders_unlimited(user_id_param uuid)
RETURNS TABLE(
  id uuid, user_id uuid, po_number text, sku_code text, quantity integer, 
  status text, order_date timestamp with time zone, expected_delivery timestamp with time zone, 
