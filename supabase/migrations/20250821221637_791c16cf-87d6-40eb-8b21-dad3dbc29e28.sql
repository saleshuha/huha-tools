...
BEGIN
    ...
    recent_upload_data AS (
        SELECT jsonb_agg(
            jsonb_build_object(
                'file_name', DISTINCT file_name,
                'upload_date', MAX(created_at),
                'order_count', COUNT(*)
            ) ORDER BY MAX(created_at) DESC
        ) FILTER (WHERE file_name IS NOT NULL) as uploads
        FROM (
            SELECT DISTINCT ON (file_name) file_name, created_at
    ...
    supplier_data AS (
        SELECT jsonb_agg(
            jsonb_build_object(
                'po_number', po_number,
                'total_value', SUM(COALESCE(total_cost, 0)),
                'order_count', COUNT(*)
