-- Add eligible_for_restock column to asin_inventory table
ALTER TABLE public.asin_inventory 
ADD COLUMN eligible_for_restock boolean DEFAULT false;

-- Add index for better query performance
CREATE INDEX idx_asin_inventory_eligible_restock ON public.asin_inventory(user_id, eligible_for_restock, quantity) WHERE eligible_for_restock = true;

-- Update the get_items_needing_restock function to include eligible_for_restock filter
...
BEGIN
  ...
  WHERE ai.quantity = 0  -- Only show items with 0 quantity
    ...
    AND ai.eligible_for_restock = true  -- Only show items marked as eligible for restock
