...
export function NoonOrdersTable({ selectedStoreId, onStoreChange }: NoonOrdersTableProps) {
  ...
  React.useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, statusFilter, selectedStoreId, sortField, sortDirection]);

  // Group orders based on view mode
  const groupedOrders = React.useMemo(() => {
    if (viewMode === 'sunsky-groups') {
      const groups = new Map<string, typeof paginatedOrders>();
    ...
    return [{ groupKey: 'All Orders', orders: paginatedOrders }];
  }, [paginatedOrders, viewMode]);
                          ...
                          return (
                            <div className="space-y-3">
                              ...
                              <div className="grid grid-cols-3 md:grid-cols-6 gap-2 text-xs">
                                ...
                                <div className="bg-purple-50 p-2 rounded border">
                                  <div className="font-semibold text-purple-800">Placed</div>
                                  <div className="text-lg font-bold text-purple-900">{metrics.sunskyPlaced}</div>
                                </div>
                                <div className="bg-green-50 p-2 rounded border">
                                  <div className="font-semibold text-green-800">Shipped</div>
                                  <div className="text-lg font-bold text-green-900">{metrics.sunskyShipped}</div>
                                </div>
                                <div className="bg-yellow-50 p-2 rounded border">
                                ...
                                <div className="bg-red-50 p-2 rounded border">
                                  <div className="font-semibold text-red-800">Breach</div>
                                  <div className="text-lg font-bold text-red-900">{metrics.breachOrders}</div>
                                </div>
                                <div className="bg-orange-50 p-2 rounded border">
                                  <div className="font-semibold text-orange-800">N Pending</div>
                                  <div className="text-lg font-bold text-orange-900">{metrics.noonPending}</div>
                                </div>
                              </div>
                              
                              {/* Progress bar */}
                              <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden flex">
                                <div className="h-full bg-green-500" style={{ width: `${metrics.sunskyShippedPercent}%` }}></div>
                                <div className="h-full bg-blue-500" style={{ width: `${metrics.sunskyProcessedPercent}%` }}></div>
                                <div className="h-full bg-yellow-500" style={{ width: `${metrics.pendingToPlaceSunskyPercent}%` }}></div>
                                <div className="h-full bg-red-500" style={{ width: `${metrics.breachOrdersPercent}%` }}></div>
                                <div className="h-full bg-orange-500" style={{ width: `${metrics.noonPendingPercent}%` }}></div>
                              </div>
                            </div>
                          );
                        })()}
