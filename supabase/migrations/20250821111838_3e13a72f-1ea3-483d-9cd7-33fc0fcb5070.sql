-- Update the get_all_po_orders function to handle large datasets without limits
...
AS $function$
DECLARE
  batch_size integer := 5000; -- Process in batches to handle large datasets
  offset_count integer := 0;
  has_more boolean := true;
BEGIN
  -- Use a cursor-based approach to handle large datasets efficiently
  RETURN QUERY
  SELECT 
    po.id,
    po.user_id,
